                    ...
                    if jrole == "مشرف":
                        for _, _jm_ap in filtered.iterrows():
                            ...
                            if _hal_ap == "تمت المناقشة" and _aq_ap and _aq_ap not in ["","nan"]:
                                ...
                                with _c3_ap:
                                    if _as_ap != "نعم":
                                        if st.button("✅ أوافق على الإيداع النهائي", key=f"ap_sup_{_mid_ap}", use_container_width=True):
    ...
    if not st.session_state.logged_in:
        ...
        if _is_library_user and tab_maktaba:
         with tab_maktaba:
            ...
            _df_lib = load_memos()
            _lib_memos = _df_lib[
                (_df_lib.get("موافقة المشرف", pd.Series()).astype(str).str.strip() == "نعم") &
                (_df_lib.get("رابط المذكرة النهائية", pd.Series()).astype(str).str.strip().apply(lambda x: x not in ["","nan"]))
            ].copy() if "موافقة المشرف" in _df_lib.columns else pd.DataFrame()
            ...
            if _lib_memos.empty:
                ...
                for _, _lrow in _lib_memos.iterrows():
                    ...
                    _ltitle = str(_lrow.get("عنوان المذكرة","")).strip()[:50]
                    _ls1 = str(_lrow.get("الطالب الأول","")).strip()
                    _laq = str(_lrow.get("رابط المذكرة النهائية","")).strip()
                    _lar = str(_lrow.get("رابط الملخص النهائي","")).strip()
                    _lau = str(_lrow.get("تبرئة المكتبة","")).strip()
